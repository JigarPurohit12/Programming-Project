...
/**
 * Closest Pair of Points - Java Implementation
 * Implements both Brute Force (O(n^2)) and Divide-and-Conquer (O(n log n)) algorithms.
 * Generates performance results for increasing input sizes.
 */
...
public class ClosestPair {
    ...
    private static void merge(Point[] points, int left, int mid, int right) {
        ...
        while (i < n1) points[k++] = leftArr[i++];
        while (j < n2) points[k++] = rightArr[j++];
    }
            ...
            for (int size : sizes) {
                ...
                for (int i = 0; i < iterations; i++) {
                    ...
                    for (int j = 0; j < size; j++) {
                        points[j] = new Point(rand.nextDouble() * 10000, rand.nextDouble() * 10000);
                    }
